...
classifier_names = ["Perceptron", "Logistic Regression", "Linear Discriminant Analysis", "Ridge",
		 "Passive Aggressive", "Linear SVM", "Polynomial SVM", "RBF SVM", "Sigmoid SVM"
		 "Nearest Neighbors", "Quadratic Discriminant Analysis", "Bernoulli Naive Bayes", "Decision Tree",
         "Random Forest", "AdaBoost", "Bagging", "Gradient Boosting", "Voting"]
...
out.write("%% Features, Reduction Technique, Classifier, Score, Error\n")

(num_data_points, num_features) = X.shape

for pct_features in pct_features_list:
	dim_reds = get_dim_reds_scikit(pct_features)
	for (dim_red_name, dim_red_technique) in zip(dim_red_names, dim_reds):
		data = dim_red_technique.fit_transform(X,Y)
		print data.shape
		for (classifier_name, classifier) in zip(classifier_names, classifiers):
			try:
				scores = cross_validation.cross_val_score(classifier, data, Y, cv=10)
				#out.write("Reduction Technique: %s, Classifier: %s, Score: %.4f \n" % (dim_red_name, classifier_name, scores.mean()))
				out.write(",".join([str(pct_features), dim_red_name, classifier_name, str(scores.mean()), ""]) + "\n")
			except ValueError as e:
				#out.write("ERROR: Reduction Technique: %s, Classifier: %s, Error: %s \n" % (dim_red_name, classifier_name, e))
				out.write(",".join([str(pct_features), dim_red_name, classifier_name, "", e]) + "\n")
